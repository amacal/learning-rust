...
extern "C" fn main(args: &'static ProcessArguments) -> ! {
    ...
    let commands: [&'static [u8]; 9] = [
        b"cat", b"faster", b"hello", b"pipe", b"sha1sum", b"spawn", b"sync", b"thread", b"tick",
    ];
    ...
    let result = match args.select(1, commands) {
        ...
        Some(b"hello") => runtime.run(|ops| {
            HelloCommand {
                msg: b"Hello, World!\n",
            }
            .execute(ops)
        }),
        Some(b"pipe") => runtime.run(|ops| {
            PipeCommand {
                msg: b"Hello, World!\n",
            }
            .execute(ops)
