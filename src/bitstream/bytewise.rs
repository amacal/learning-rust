...
pub struct BitStreamBytewise<const T: usize> {
...
impl<const T: usize> BitStreamBytewise<T> {
    pub fn new() -> Self {
        Self {
            buffer: Box::new([0; T]),
            boundary: 0,
...
impl<const T: usize> BitStream for BitStreamBytewise<T> {
...
mod tests {
    ...
    fn creates_bitstream() {
        let bitstream: BitStreamBytewise<32> = BitStreamBytewise::new();
    ...
    fn appends_too_big_slice() {
        let mut bitstream: BitStreamBytewise<2> = BitStreamBytewise::new();
        let data = [0, 1, 2, 3];
    ...
    fn reads_bit_by_bit() {
        let data = [0x27, 0x00];
        let mut bitstream: BitStreamBytewise<32> = bitstream(&data);
